...
class ClientsListWindow(QWidget):
    def __init__(self):
        ...
        if self.clients is None:
            ...
            self.all_clients = sorted(
                self.clients, key=lambda client: client.full_name)
    ...
    def ui_clients_list_window(self):
        ...
        self.user_info_button.setStyleSheet(
            "background-color: rgb(30, 138, 86); font-size: 14px; color: white; border: 0; border-radius: 4px;")
        ...
        self.clients_table.setHorizontalHeaderLabels(
            ["ФИО", "Дата рождения", "Телефон", "Е-mail"])
        ...
        self.clients_table.setEditTriggers(
            QTableWidget.EditTrigger.NoEditTriggers)
        self.clients_table.cellDoubleClicked.connect(lambda: open_client_info_window(
            self, self.clients[self.clients_table.currentRow()].id))

        self.populate_clients_table(self.clients)

        self.horizontalLayoutWidget_2 = QWidget(self)
        self.horizontalLayoutWidget_2.setGeometry(
            QtCore.QRect(16, 35, 320, 39))
        ...
        self.lineEdit.setPlaceholderText(
            "Введите фамилию/имя/отчество клиента")
        self.horizontalLayout_2.addWidget(self.lineEdit)

        self.search_btn = QPushButton("Поиск", self.horizontalLayoutWidget_2)
        self.search_btn.setFixedSize(45, 20)
        self.search_btn.setStyleSheet(
            "background-color: rgb(30, 138, 86); font-size: 15x; color: white; border: 0; border-radius: 4px;")
